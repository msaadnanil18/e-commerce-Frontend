...
export function NewTableHOC<T extends object>({
  ...
  const prevPageIndexRef = useRef(serverSidePagination?.pageIndex || 0);
  const prevPageSizeRef = useRef(serverSidePagination?.pageSize || pageSize);
  const prevSortByRef = useRef<SortingRule<object>[]>([]);
  ...
  const handlePageChange = useCallback(
    (newPageIndex: number) => {
      if (!serverSidePagination || !serverSidePagination.onPageChange) return;

      // Only trigger if the page has actually changed
      if (newPageIndex !== prevPageIndexRef.current) {
        serverSidePagination.onPageChange(newPageIndex + 1);
        prevPageIndexRef.current = newPageIndex;
      }
    },
    [serverSidePagination]
  );
  ...
  const handlePageSizeChange = useCallback(
    (newPageSize: number) => {
      if (!serverSidePagination || !serverSidePagination.onPageSizeChange)
        return;

      // Only trigger if the page size has actually changed
      if (newPageSize !== prevPageSizeRef.current) {
  ...
  const handleSortChange = useCallback(
    (newSortBy: SortingRule<object>[]) => {
      if (!serverSideSorting || !serverSideSorting.onSortChange) return;

      // Only trigger if the sort has actually changed
      const prevSortByStr = JSON.stringify(prevSortByRef.current);
      const newSortByStr = JSON.stringify(newSortBy);

      if (prevSortByStr !== newSortByStr) {
        serverSideSorting.onSortChange(newSortBy);
  ...
  // Optimized useEffects with proper dependency tracking
  useEffect(() => {
    if (serverSidePagination && !serverSideSorting) {
      handlePageChange(pageIndex);
    }
  }, [pageIndex, handlePageChange, serverSideSorting]);
